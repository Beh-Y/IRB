/*
 * PCDF (Publication Checklist Declaration Form) document generation script.
 * Standalone form (no parent record) with the simplest workflow of the
 * three: Draft -> S/D Director Approval -> Approved. There's no Secretariat
 * or IRB Member review stage at all -- the Director's approval is the only
 * gate -- and Approved ends with the same PI Acknowledge step as the IPAF.
 */

class PcdfFormController {
  constructor(record, currentRole) {
    this.record = record;
    this.currentRole = currentRole;
    this.fields = flattenFields(PCDF_SCHEMA);
    this.fieldEls = {};
  }
  ...
  buildControl(field, controlWrap) {
    ...
    if (field.type === 'yesno' || field.type === 'yesna') {
  ...
  onFieldChanged(field) {
    ...
    } else if (field.type === 'yesno' || field.type === 'yesna') {
